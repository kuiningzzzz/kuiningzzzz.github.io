...
之后点击vscode下边框左下角的运行按钮（如果没有的话就重启一下vscode，可能是插件还没加载出来）。如果上面弹出了编译器选项的话，在弹出的选项里选择你的编译器即可（比如我们在入门篇安装的mingw）。如果需要打断点调试，就点左下角运行按钮旁边的小虫子按钮。

此时如果你的程序开始启动，在目录下生成了一个build文件夹，并且成功在控制台输出了helloworld，那么恭喜你完成了Cmake的初步配置！（至少是能用了）

### 第四步：CmakeLists.txt的更多设置
